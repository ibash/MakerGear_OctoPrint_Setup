...
def copyAndLog(fromFile, toFile, logfile):
    try:
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "Copy Environmental Error on " + fromFile + " to " + toFile + "\n"
        )
        logfile.write(str(e))
        logfile.write("")
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "Copy  Non Enviromental Error on " + fromFile + " to " + toFile + "\n"
        )
        logfile.write(str(e))
        logfile.write("---------------\n")
    else:
        logfile.write(
            "File copied from " + fromFile + " to " + toFile + " sucesfully\n"
        )
...
def chmodAndLog(file, permission, logfile):
    try:
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "CHMOD Environmental Error on " + file + " to " + str(permission) + "\n"
        )
        logfile.write(str(e))
        logfile.write("---------------\n")
    except Exception as e:
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "CHMOD  Non Enviromental Error on " + file + " to " + str(permission) + "\n"
        )
        logfile.write(str(e))
        logfile.write("---------------\n")
    else:
...
def chownAndLog(file, uid, gid, logfile):
    try:
        ...
        logfile.write("-----ERROR-----\n")
        logfile.write(
            "CHOWN Non Enviromental Error on " + file + " to " + str(uid) + "\n"
        )
        logfile.write(str(e))
        logfile.write("---------------\n")
    else:
...
def tap():
    ...
    if avrFlag == 0:
        ...
        logText = (
            dateTimeVal
            + "Cancel script was activated because the RAMBo reset button  / reset line was tapped. This could also occur because of a software reset RAMBo command"
        )
        persistentLogFileName = (
            "/home/pi/.octoprint/logs/cancel_and_shutdown_from_reset.log"
        )
...
def resetPasswordHold():
    ...
    copyAndLog(
        "/home/pi/.octoprint/users.yaml.backup",
        "/home/pi/.octoprint/users.yaml",
        logfile,
    )
    copyAndLog(
        "/home/pi/.octoprint/scripts/interfaces", "/etc/network/interfaces", logfile
    )
    copyAndLog("/etc/netconnectd.yaml.backup", "/etc/netconnectd.yaml", logfile)
    copyAndLog(
        "/home/pi/.octoprint/scripts/config.txt.original", "/boot/config.txt", logfile
    )
    ...
    logText = (
        dateTimeVal
        + "System was restored because the RAMBo reset button  / reset line was held for more than 60 seconds at See "
        + logFileName
