...
for line in os.popen('pgrep -x "avrdude"'):
    print(
        "Warning: AVRdude is already running, Quitting upload process. You may need to restart the printer to clear this warning."
    )
    sys.stdout.flush()
    quit()

subprocess.check_output(
    "echo $(date) > /home/pi/.octoprint/logs/firmware.log", shell=True
)
...
try:
    subprocess.check_output(
        "platformio run >> /home/pi/.octoprint/logs/firmware.log", shell=True
    )
...
ser = serial.Serial(
    "/dev/ttyS0", 115200, timeout=0.5, xonxoff=False, rtscts=False, dsrdtr=False
)  # start serial port to upload script
...
if m206X != None and m206Y != None and m206Z != None:
    ...
    print(
        "Could not read previous M503 M206 - Please proceed to Quick Check after firmware has uploaded"
    )
    subprocess.check_output(
...
if extruderCount == None:
    print(
        "Could not read number of extruders - Please proceed to Quick Check after firmware has uploaded"
    )
    subprocess.check_output(
    ...
    if m218X != None and m218Y != None and m218Z != None:
        ...
        print(
            "Could not read previous M503 M218 - Please proceed to Quick Check after firmware has uploaded"
        )
        subprocess.check_output(
...
if zprobe == None:
    print(
        "Could not read previous M503 M851 - Please proceed to Quick Check after firmware has uploaded"
    )
...
subprocess.check_output(
    "echo Starting Upload >> /home/pi/.octoprint/logs/firmware.log", shell=True
)
sys.stdout.flush()
...
try:
    ...
    subprocess.check_output(
        "echo Upload Sucessful >> /home/pi/.octoprint/logs/firmware.log", shell=True
    )
    sys.stdout.flush()
except subprocess.CalledProcessError:
...
if matcher != None:
    ...
    subprocess.check_output(
        "echo Restoring parameters >> /home/pi/.octoprint/logs/firmware.log", shell=True
    )
    sys.stdout.flush()

    ser = serial.Serial(
        "/dev/ttyS0", 115200, timeout=2, xonxoff=False, rtscts=False, dsrdtr=False
    )  # start serial port to upload script
    ser.flushInput()  # clear any latent input
    ser.flushOutput()  # clear any latent output
    ...
    if m851String != None:
        ...
        subprocess.check_output(
            "echo M851 Z" + m851Z + " >> /home/pi/.octoprint/logs/firmware.log",
            shell=True,
        )
        # print "write M851"
